...
def create_multimodal_pipeline_visualization():
    ...
    # Title
    ax.text(5, 9.5, 'Multimodal AI Pipeline', fontsize=16, fontweight='bold', ha='center')
    ...
    for inp in inputs:
        ...
        ax.add_patch(rect)
        ax.text(inp['pos'][0], inp['pos'][1], inp['name'], 
    ...
    for enc in encoders:
        ...
        ax.add_patch(rect)
        ax.text(enc['pos'][0], enc['pos'][1], enc['name'], 
    ...
    ax.add_patch(rect)
    ax.text(5, 3.1, 'Shared Embedding Space', 
    ...
    # Draw arrows
    # Inputs to encoders
    for i, (inp, enc) in enumerate(zip(inputs, encoders)):
        ax.arrow(inp['pos'][0], inp['pos'][1]-0.4, 
                0, -0.8, 
                head_width=0.1, head_length=0.1, fc='black', ec='black')
    
    # Encoders to embedding space
    for enc in encoders:
        ax.arrow(enc['pos'][0], enc['pos'][1]-0.4, 
                0, -0.8, 
                head_width=0.1, head_length=0.1, fc='purple', ec='purple')
    
    # Embedding space to applications
    for app in apps:
        ax.arrow(app['pos'][0], 2.5, 
                0, -1.0, 
                head_width=0.05, head_length=0.05, fc='darkgreen', ec='darkgreen',
...
def create_diffusion_process_visualization():
    ...
    fig.suptitle('Diffusion Process: From Noise to Image', fontsize=14, fontweight='bold')
    plt.tight_layout()
...
def create_model_comparison_chart():
    ...
    for i in range(len(models)):
        for j in range(len(capabilities)):
            text = ax.text(j, i, f'{capability_matrix[i, j]:.1f}',
                          ha="center", va="center", color="black" if capability_matrix[i, j] < 0.5 else "white")
    
    ax.set_title('Multimodal Model Capabilities Comparison', fontsize=14, fontweight='bold', pad=20)
